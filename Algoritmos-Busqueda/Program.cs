...
class Program
{
    ...
    static int BusquedaBinaria(int[] arr, int objetivo, out int iteraciones)
    {
        ...
        {
            iteraciones++;
            // Variante segura para calcular el centro (evita desbordamientos de memoria)
            int centro = izquierda + (derecha - izquierda) / 2;

            if (arr[centro] == objetivo)
                return centro;

            // Lógica de descarte asegurando que los límites se actualicen (evita bucles infinitos)
            if (arr[centro] < objetivo)
                izquierda = centro + 1;
            else
                derecha = centro - 1;
